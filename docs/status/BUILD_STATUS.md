...
- **Config-driven column pattern** established on Standings (shared `cellStyle` driving both header and rows; commit `07aaa78`) and now **ported to the RosterView table** (step 2 PART A): `cellStyle` is extracted/exported from `PlayerRow` (with `minWidth:0` on the flex column) and drives both the hand-rolled header and the PlayerRow body off one tier-aware column array. Apply to other tables (cap, contracts, transactions) as they're next touched.
