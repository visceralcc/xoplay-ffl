...
- **Current Matchup (placeholder) is built, signed off, and committed** (2026-06-05, `ae3fd8f`). First Franchise-area matchup surface, off `Screen_Matchup.md` (specs/franchise/screens/): `src/screens/Matchup.tsx`, registered in the preview (Screens → "Matchup", phone/compact, week-11 BRO vs OAK live, BRO marked "your team"; `neutral` + `noMatchup` toggles). Three regions in stacking order: **MatchupCard summary** (away-left / home-right, stored `Matchup` totals, status-aware — LiveDot when IN_PROGRESS; null score → "—"), then **away lineup** + **home lineup** `Section`s, each a `DataTable` of that side's starters (via `getLineupForMatchup` filtered to `isStarter`) — row = PlayerRow identity cell + the entry's stored `fantasyPoints` (`=== null` → "—", real `0.0` shows "0.0"). No franchise masthead. Owner `onSetLineup` is **stubbed**. Deferred (Out of scope): slot-by-slot comparison row (would need a new component), per-rule scoring breakdown, win probability, live refresh, bench, week navigation
...
- **Next (options):** (a) **Salary Cap Overview (placeholder)** — clean, non-blocked: `CapMeter` already exists for it, contracts render through `DataTable`, cap usage/room helpers exist; only dead money defers (no `SalaryAdjustment` in the fixture). Dynasty-gated. (b) **Lineup Submission** — the most important mobile screen, but **not blocked on the Supabase project** as previously noted here: per-player projections don't come from the Stats Service (it provides none — XO Play sources them, per Stats Service Consumer OQ3), and placeholder screens read the fixture, not Supabase. Its real prerequisites are local — a new **`LineupSlots`** component (slot assignment; the placeholder convention says spec-then-build a new component, don't improvise one) plus two small fixture gaps (a mock projection field; no lineup-lock "now" notion → defer the live lock/countdown). Path: spec `LineupSlots` → build it → then stamp the screen. Confirm scope at session start
...
| Current Matchup — placeholder (Screen_Matchup) | ✅ |
...
- **Config-driven column pattern — now unified on `DataTable`'s header.** Originally established on Standings with a hand-rolled header + shared `cellStyle` (commit `07aaa78`) and ported to RosterView (step 2 PART A). As of 2026-06-04 the standings tables (`Standings`, `LeagueHome`) no longer hand-roll a header — they use `DataTable`'s built-in header, hardened with `minWidth:0` on the flex column so header and rows can't drift. RosterView still uses the exported `cellStyle` from `PlayerRow`; fold it onto `DataTable`'s header too when next touched, for full consistency. `Matchup`'s lineup tables (2026-06-05) are a **second consumer** of the same PlayerRow-identity-under-`DataTable`-header pattern — a PlayerRow identity cell in a flex region + a trailing points cell — so the flex player column's right edge drifts a few px from the header cell (the row carries PlayerRow's `md` padding vs. the header's column gap); the **points column still aligns exactly**. Fold both `RosterView` and `Matchup` onto `DataTable`'s header in the same unification pass. **Layer hardened (structural cleanup):** the trailing action column is now reserved in BOTH owner and visitor views (visitor renders an empty spacer of the same width — `ACTION_COL_WIDTH`, exported from `PlayerRow`) so toggling `isOwner` moves nothing but the affordance; the flex name column truncates with an ellipsis before the numeric block (`nameTeam` stretches + `minWidth:0`); the 16px injury badge holds a fixed slot immediately after the name; numerics are clustered via a tightened `COLUMN_GAP` (PlayerRow gained an optional `gap` prop) + trimmed widths; ~20px right-edge gutter carried inside the reserved action cell so the header fill still bleeds to the edge. Gap / widths / gutter are **PLACEHOLDER spacing — Figma supersedes**; header typography + action-menu visual design still await Figma.
